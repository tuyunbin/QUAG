...
class MomentDataset(Dataset):
    def __init__(self, args, data_path, video_dir=None, video_feature_dir=None, asr_dir=None, asr_feature_dir=None,
                 n_model_frames=-1, task=None):
        ...
        for i, (prompt, video_anns) in enumerate(prompt2video_anns.items()):
            ...
            for video_fname, video_ann in video_anns.items():
                ...
                if task == 'moment_retrieval':
                    ...
                    start_frame = timestamp_to_frame_index(moment_start, video_duration=video_duration,
                                                           n_frames=n_frames)
                    ...
                    approximate_bounds += [
                        [frame_index_to_timestamp(start_frame, video_duration=video_duration, n_frames=n_frames),
                    ...
                    if 'train' in str(data_path):
                        ...
                        moment_start_frame = timestamp_to_frame_index(moment_start, video_duration=video_duration,
                                                                      n_frames=n_frames)
                        moment_end_frame = timestamp_to_frame_index(moment_end, video_duration=video_duration,
                                                                    n_frames=n_frames)
                        ...
                        all_boundaries_frames = [
                            timestamp_to_frame_index(bound, video_duration=video_duration, n_frames=n_frames) for bound
                            in all_boundaries]
                        ...
                        for i in range(len(all_boundaries) - 1):
                            ...
                            step_start_frame = timestamp_to_frame_index(start, video_duration=video_duration,
                                                                        n_frames=n_frames)
                            step_end_frame = timestamp_to_frame_index(end, video_duration=video_duration,
                                                                      n_frames=n_frames)
                    ...
                    for step in video_ann['steps']:
                        ...
                        start_frame = timestamp_to_frame_index(step_start, video_duration=video_duration,
                                                               n_frames=n_frames)
        ...
        if self.video_feature_dir is not None:
            ...
            if self.n_model_frames > 0:
                ...
                if n_frames > self.n_model_frames:
                    ...
                    count_embeds = [count_embeds[(j * self.n_model_frames) // N: ((j + 1) * self.n_model_frames) // N]
                                    for j in range(N)]
    ...
    def collate_fn(self, batch):
        ...
        prompt_mask = torch.zeros_like(clip_text_ids)
        nonzeros = np.array(list(map(lambda x: (x != 0).sum(), clip_text_ids)))
        for ix, row in enumerate(prompt_mask):
            row[:nonzeros[ix]] = 1

        out_batch['clip_text_ids'] = clip_text_ids
        out_batch['clip_text_masks'] = prompt_mask
